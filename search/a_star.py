...
def astar_search(
    ...
    while open:
        ...
        if pop_g == state_cost[pop_state]:
            ...
            for operation, successor_state in task.get_successor_states(pop_state):
                neighbor_node = searchspace.make_child_node(pop_node, operation, successor_state)
                ...
                old_g = state_cost.get(successor_state, float("inf"))
                neighbor_state = neighbor_node.state
                neighbor_g = neighbor_node.g
                if old_g > neighbor_node.g:
                    node_tiebreaker += 1
                    state_cost[neighbor_state] = neighbor_g
